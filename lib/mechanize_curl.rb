...
require 'mechanize_curl/version'
require 'curl'
require 'fileutils'
require 'forwardable'
...
#
# The error raised, MechanizeCurl::ResponseReadError, can be converted to a parsed
...
#   begin
#     page = agent.get uri
#   rescue MechanizeCurl::ResponseReadError => e
#     page = e.force_parse
#   end
...
class MechanizeCurl
  ...
  #
  #    agent_1 = MechanizeCurl.new 'conn1'
  #    agent_2 = MechanizeCurl.new 'conn2'
  #

  def initialize(connection_name = 'mechanize')
    @agent = MechanizeCurl::HTTP::Agent.new(connection_name)
    @agent.context = self
    @log = nil
  ...
  # the MechanizeCurl::Page::Link object passed in.  Returns the page fetched.
  ...
  # #get.
  #
  # By default, if the Content-type of the response matches a MechanizeCurl::File
  # or MechanizeCurl::Page parser, the response body will be loaded into memory
  # before being saved.  See #pluggable_parser for details on changing this
  # default.
  #
  # For alternate ways of downloading files see MechanizeCurl::FileSaver and
  # MechanizeCurl::DirectorySaver.
  ...
  # MechanizeCurl::Util.build_query_string, which see.
  #
  ...
  def get(uri, parameters = [], referer = nil, headers = {})
    ...
    # MechanizeCurl::Page#base
    unless MechanizeCurl::Parser === referer then
  ...
  #
  # +query+ is processed using MechanizeCurl::Util.each_parameter (which
  ...
  def post(uri, query = {}, headers = {})
    ...
    MechanizeCurl::Util.each_parameter(query) { |k, v|
      if v.is_a?(IO)
  ...
  class << self
    ...
    #
    #   MechanizeCurl.html_parser = Nokogiri::XML
    ...
    #
    #   MechanizeCurl.log = Logger.new $stderr
  ...
  # or a media type 'text'.  See MechanizeCurl::PluggableParser for further
  # details.
  #
  # Example:
  #
  #   agent.pluggable_parser['application/octet-stream'] = MechanizeCurl::Download
...
Use of #auth and #basic_auth are deprecated due to a security vulnerability.
  ...
  ##
  # A MechanizeCurl::CookieJar which stores cookies
  ...
  ##
  # The current logger.  If no logger has been set MechanizeCurl.log is used.

  def log
    @log || MechanizeCurl.log
  end
  ...
  # memory.
  #
  # Note that for MechanizeCurl::Download subclasses, the maximum buffer size
  ...
  def parse uri, response, body
    ...
    unless parser_klass <= MechanizeCurl::Download then
      body = case body
...
require 'mechanize_curl/element_not_found_error'
require 'mechanize_curl/response_read_error'
require 'mechanize_curl/chunked_termination_error'
require 'mechanize_curl/content_type_error'
require 'mechanize_curl/cookie'
require 'mechanize_curl/cookie_jar'
require 'mechanize_curl/parser'
require 'mechanize_curl/download'
require 'mechanize_curl/directory_saver'
require 'mechanize_curl/file'
require 'mechanize_curl/file_connection'
require 'mechanize_curl/file_request'
require 'mechanize_curl/file_response'
require 'mechanize_curl/form'
require 'mechanize_curl/history'
require 'mechanize_curl/http'
require 'mechanize_curl/http/agent'
require 'mechanize_curl/http/auth_challenge'
require 'mechanize_curl/http/auth_realm'
require 'mechanize_curl/http/content_disposition_parser'
require 'mechanize_curl/http/www_authenticate_parser'
require 'mechanize_curl/image'
require 'mechanize_curl/page'
require 'mechanize_curl/pluggable_parsers'
require 'mechanize_curl/redirect_limit_reached_error'
require 'mechanize_curl/redirect_not_get_or_head_error'
require 'mechanize_curl/response_code_error'
require 'mechanize_curl/robots_disallowed_error'
require 'mechanize_curl/unauthorized_error'
require 'mechanize_curl/unsupported_scheme_error'
require 'mechanize_curl/util'
