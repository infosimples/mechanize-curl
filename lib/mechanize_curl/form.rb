...
require 'mechanize_curl/element_matcher'
...
#
#   form = page.forms.first # => MechanizeCurl::Form
...
class MechanizeCurl::Form
  extend Forwardable
  extend MechanizeCurl::ElementMatcher
  ...
  def initialize(node, mech = nil, page = nil)
    ...
    @node             = node
    @action           = MechanizeCurl::Util.html_unescape(node['action'])
  ...
  def from_native_charset str
    MechanizeCurl::Util.from_native_charset(str, encoding, @ignore_encoding_error,
                                        @mech && @mech.log)
  end
  ...
  def build_query(buttons = [])
    ...
    (fields + checkboxes).reject do |f|
      ...
      case f
      when MechanizeCurl::Form::CheckBox
        if f.checked
          successful_controls << f
        end
      when MechanizeCurl::Form::Field
        successful_controls << f
      end
    end
  ...
  def request_data
    ...
    when /^multipart\/form-data/
      ...
      MechanizeCurl::Util.build_query_string(query_params)
    end
  end
...
require 'mechanize_curl/form/field'
require 'mechanize_curl/form/button'
require 'mechanize_curl/form/hidden'
require 'mechanize_curl/form/text'
require 'mechanize_curl/form/textarea'
require 'mechanize_curl/form/submit'
require 'mechanize_curl/form/reset'
require 'mechanize_curl/form/file_upload'
require 'mechanize_curl/form/keygen'
require 'mechanize_curl/form/image_button'
require 'mechanize_curl/form/multi_select_list'
require 'mechanize_curl/form/option'
require 'mechanize_curl/form/radio_button'
require 'mechanize_curl/form/check_box'
require 'mechanize_curl/form/select_list'
