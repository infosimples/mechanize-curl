...
class MechanizeCurl::Page::Image
  ...
  ##
  # Creates a new MechanizeCurl::Page::Image from an image +node+ and source
  # +page+.
  ...
  def image_referer # :nodoc:
    ...
    when https_page && relative? then page
      ...
      MechanizeCurl::File.new(nil, { 'content-type' => 'text/plain' }, '', 200)
    end
  end
  ...
  def mime_type
    ...
    MechanizeCurl::Util::DefaultMimeTypes[suffix_without_dot]
  end
  ...
  def url
    if relative? then
      if page.bases[0] then
        ...
        page.uri + MechanizeCurl::Util.uri_escape(src.to_s)
      end
    else
      URI MechanizeCurl::Util.uri_escape(src)
    end
  end
