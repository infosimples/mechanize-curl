...
class MechanizeCurl::HTTP::Agent
  ...
  # raising MechanizeCurl::ResponseCodeError, defaulted to an empty array.
  ...
  def initialize(connection_name = 'mechanize')
    ...
    @cookie_jar               = MechanizeCurl::CookieJar.new
    ...
    # HTTP Authentication
    @auth_store           = MechanizeCurl::HTTP::AuthStore.new
    @authenticate_parser  = MechanizeCurl::HTTP::WWWAuthenticateParser.new
    ...
    @scheme_handlers = Hash.new { |h, scheme|
      h[scheme] = lambda { |link, page|
        raise MechanizeCurl::UnsupportedSchemeError.new(scheme, link)
      }
    }
  ...
  def fetch uri, method = :get, headers = {}, params = [],
    ...
    if robots && uri.is_a?(URI::HTTP)
      robots_allowed?(uri) or raise MechanizeCurl::RobotsDisallowedError.new(uri)
    end
    ...
    if robots && page.is_a?(MechanizeCurl::Page)
      page.parser.noindex? and raise MechanizeCurl::RobotsDisallowedError.new(uri)
    end
    ...
    if curl.response_code.to_s[0] == '2'
      ...
      if @allowed_error_codes.any? {|code| code.to_s == page.code} then
        ...
        raise MechanizeCurl::ResponseCodeError.new(page, 'unhandled response')
      end
    end
  end
  ...
  def connection_for uri
    ...
    when 'http', 'https' then
      return @http
    when 'file' then
      return MechanizeCurl::FileConnection.new
    end
  end
  ...
  def content_encoding_brotli(body_io)
    ...
    unless defined?(::Brotli)
      raise MechanizeCurl::Error, "cannot deflate brotli-encoded response. Please install and require the 'brotli' gem."
    end
    ...
    begin
      ...
      raise MechanizeCurl::Error, "error inflating brotli-encoded response."
    end
  ensure
    body_io.close
  end
  ...
  def content_encoding_zstd(body_io)
    ...
    unless defined?(::Zstd)
      raise MechanizeCurl::Error, "cannot deflate zstd-encoded response. Please install and require the 'zstd-ruby' gem."
    end
    ...
    begin
      ...
      raise MechanizeCurl::Error, "error decompressing zstd-encoded response."
    end
  ensure
    body_io.close
  end
  ...
  def http_request uri, method, params = nil
    ...
    when 'http', 'https' then
      ...
      request
    when 'file' then
      MechanizeCurl::FileRequest.new uri
    end
  end
  ...
  def resolve(uri, referer = current_page)
    ...
    if uri.is_a?(URI)
      ...
      url.gsub!(/[^#{0.chr}-#{126.chr}]/o) { |match|
        MechanizeCurl::Util.uri_escape(match)
      }

      escaped_url = MechanizeCurl::Util.html_unescape(
  ...
  def secure_resolve!(uri, referer = current_page)
    ...
    if (referer_uri = referer && referer.uri) &&
      ...
      raise MechanizeCurl::Error, "insecure redirect to a file URI"
    end
  ...
  def resolve_parameters uri, method, parameters
    ...
    when :head, :get, :delete, :trace then
      if parameters and parameters.length > 0
        ...
        uri.query << MechanizeCurl::Util.build_query_string(parameters)
      end
  ...
  def get_meta_refresh response, uri, page
    ...
    if page.respond_to?(:meta_refresh) and
      ...
      delay, href, link_self = MechanizeCurl::Page::MetaRefresh.parse refresh, uri
      raise MechanizeCurl::Error, 'Invalid refresh http header' unless delay
  ...
  def response_authenticate(response, page, uri, request, headers, params,
    ...
    unless www_authenticate = response['www-authenticate'] then
      ...
      raise MechanizeCurl::UnauthorizedError.new(page, nil, message)
    end
    ...
    unless @auth_store.credentials? uri, challenges then
      ...
      raise MechanizeCurl::UnauthorizedError.new(page, challenges, message)
    end
  ...
  def response_content_encoding response, body_io
    ...
    raise MechanizeCurl::Error, message
  ensure
    begin
  ...
  def response_cookies response, uri, page
    if MechanizeCurl::Page === page and page.body =~ /Set-Cookie/n
  ...
  def response_follow_meta_refresh response, uri, page, redirects
    ...
    raise MechanizeCurl::RedirectLimitReachedError.new(page, redirects) if
      redirects + 1 > @redirection_limit

    sleep delay
    @history.push(page, page.uri)
    fetch new_url, :get, {}, [],
          MechanizeCurl::Page.new, redirects + 1
  end
  ...
  def response_read response, request, uri
    ...
    begin
      ...
      body_io.rewind
      raise MechanizeCurl::ChunkedTerminationError.new(e, response, body_io, uri,
                                                   @context)
    rescue Net::HTTP::Persistent::Error, Errno::ECONNRESET => e
      body_io.rewind
      raise MechanizeCurl::ResponseReadError.new(e, response, body_io, uri,
                                             @context)
    end
    ...
    raise MechanizeCurl::ResponseCodeError.new(response, uri) if
    ...
    unless Net::HTTP::Head === request or Net::HTTPRedirection === response then
      if content_length and content_length != body_io.length
        ...
        raise MechanizeCurl::ResponseReadError.new(err, response, body_io, uri,
                                                @context)
      end
    end
  ...
  def get_robots(uri) # :nodoc:
    robots_mutex.synchronize do
      ...
      begin
        fetch(uri).body
      rescue MechanizeCurl::ResponseCodeError => e
      ...
      rescue MechanizeCurl::RedirectLimitReachedError
        ''
      ensure
...
require 'mechanize_curl/http/auth_store'
