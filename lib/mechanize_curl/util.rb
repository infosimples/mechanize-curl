...
class MechanizeCurl::Util
  ...
  #   MechanizeCurl::Util::DefaultMimeTypes.replace(mimetab)
  DefaultMimeTypes = WEBrick::HTTPUtils::DefaultMimeTypes

  class << self
    # Builds a query string from a given enumerable object
    # +parameters+.  This method uses MechanizeCurl::Util.each_parameter
  ...
  def self.from_native_charset(s, code, ignore_encoding_error = false, log = nil)
    ...
    return s unless MechanizeCurl.html_parser == Nokogiri::HTML
  ...
  def self.html_unescape(s)
    ...
    s.gsub(/&(\w+|#[0-9]+);/) { |match|
      number = case match
               when /&(\w+);/
                 MechanizeCurl.html_parser::NamedCharacters[$1]
               when /&#([0-9]+);/
                 $1.to_i
               end
