...
# MechanizeCurl::TestCase for your tests will create an isolated mechanize
# instance that won't pollute your filesystem or other tests.
#
# Once MechanizeCurl::TestCase is loaded no HTTP requests will be made outside
...
class MechanizeCurl::TestCase < Minitest::Test
  ...
  def fake_page agent = @mech
    ...
    MechanizeCurl::Page.new uri, nil, html, 200, agent
  end
  ...
  ##
  # Creates a MechanizeCurl::Page with the given +body+
  ...
  ##
  # Creates a MechanizeCurl::CookieJar by parsing the given +str+

  def cookie_jar str, uri = URI('http://example')
    MechanizeCurl::CookieJar.new.tap do |jar|
      jar.parse str, uri
    end
  end
  ...
  ##
  # Creates a MechanizeCurl::Page for the given +uri+ with the given
  # +content_type+, response +body+ and HTTP status +code+

  def page uri, content_type = 'text/html', body = String.new, code = 200
    uri = URI uri unless URI::Generic === uri

    MechanizeCurl::Page.new(uri, { 'content-type' => content_type }, body, code,
                        @mech)
  end
...
require 'mechanize_curl/test_case/servlets'
...
class Net::HTTP # :nodoc:
  ...
  def request(req, *data, &block)
    ...
    MechanizeCurl::TestCase::REQUESTS << req
    ...
    if servlet_klass = MECHANIZE_TEST_CASE_SERVLETS[path]
      ...
      unless PAGE_CACHE[filename]
        ::File.open("#{MechanizeCurl::TestCase::TEST_DIR}/#{filename}", 'rb') do |io|
