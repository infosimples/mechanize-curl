...
module MechanizeCurl::Parser
  ...
  ##
  # The MechanizeCurl::Headers for this file
  ...
  def extract_filename full_path = @full_path
    ...
    if (disposition = @response['content-disposition'])
      content_disposition =
        MechanizeCurl::HTTP::ContentDispositionParser.parse disposition
  ...
  ##
  # Creates a MechanizeCurl::Header from the Net::HTTPResponse +response+.
  #
  # This allows the Net::HTTPResponse to be garbage collected sooner.

  def fill_header response
    @response = MechanizeCurl::Headers.new
