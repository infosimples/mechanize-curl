...
class TestMechanizePageEncoding < MechanizeCurl::TestCase
  ...
  def util_page body = @body, headers = @response_headers
    MechanizeCurl::Page.new @uri, headers, body && body.force_encoding(Encoding::BINARY), 200, @mech
  end

  def test_page_charset
    charset = MechanizeCurl::Page.charset 'text/html;charset=vAlue'
    assert_equal 'vAlue', charset

    charset = MechanizeCurl::Page.charset 'text/html;charset=vaLue, text/html'
    assert_equal 'vaLue', charset
  ...
  def test_page_charset_upcase
    charset = MechanizeCurl::Page.charset 'TEXT/HTML;CHARSET=UTF-8'
  ...
  def test_page_charset_no_chaset_token
    charset = MechanizeCurl::Page.charset 'text/html'
    assert_nil charset
  end

  def test_page_charset_returns_nil_when_charset_says_none
    charset = MechanizeCurl::Page.charset 'text/html;charset=none'

    assert_nil charset
  end

  def test_page_charset_multiple
    charset = MechanizeCurl::Page.charset 'text/html;charset=111;charset=222'
  ...
  def test_page_response_header_charset
    ...
    charsets = MechanizeCurl::Page.response_header_charset(headers)
  ...
  def test_page_meta_charset
    ...
    charsets = MechanizeCurl::Page.meta_charset(body)
